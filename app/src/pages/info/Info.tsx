...
const Info: React.FC = () => {
  ...
  const onFileChange = (newFileList: FileListProps[]) => {
    ...
    newFileList.forEach(async v => {
      if (v.status === 'success') {
        ...
        try {
          await post('/api/user/update', { key: 'avatar', value: v.url, phone: user.phone })
  ...
  // 更新用户信息
  const update = async () => {
    const res: any = await post('/api/user/update', { key: type, value, phone: user.phone })
    if (res && res.code === 0) {
      const copy = JSON.parse(JSON.stringify(user))
      copy[type] = value
      setUser(copy)
      ...
      <div className="content">
        ...
        <div className="panel">
          <div className="item item-msg" style={{ flexDirection: 'column' }}>
            ...
            <div className="cell border-bottom" style={{ paddingBottom: '10px', paddingTop: '10px' }} onClick={() => history.push('/friend')}>
              <p className="msg">好友</p>
              <Icon name="right" color="#999" />
            </div>
            <div className="cell border-bottom" style={{ paddingBottom: '10px', paddingTop: '10px' }} onClick={() => history.push('/requestlist')}>
              <p className="msg">
                请求 {showDot ? <span className="dot"></span> : null} 
              </p>
              <Icon name="right" color="#999" />
            </div>
